...
class MoviesController extends Controller {
    ...
    public function topMovies($page = 1) {


        $test1 = Movie::leftJoin('ratings', function($join){
            $join->on('movies.id', '=', 'ratings.movie_id');
        })
            ->groupBy('movies.id')
            ->orderBy('ratings.rating_value','desc')
            ->get([
                'movies.id',
                'movies.title',
                'movies.year',
...
//        $test2 = DB::select(DB::raw('SELECT * FROM movie3.movies as mov
//                                    left join movie3.ratings as rat on rat.movie_id = mov.id
//                                    group by mov.title
//                                    order by rat.rating_value desc'));

//        $test3 = collect($test2);
//
//        $test = Movie::orderBy('created_at', 'desc')->get()->take(10);
...
//        $movie = $test1;
        ...
        //return view('toprating', compact('movie'));

        return $this->movies_view($test1 , null, 'Top Rated', $page, 'top_movies', []);
